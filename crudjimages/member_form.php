...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0) {
        ...
        $save = $master->insert_to_json_with_image(); // Cambiar a la función que maneja imágenes
    }
    if (isset($save['status'])) {
        if ($save['status'] == 'success') {
            if (isset($_POST['id']) && is_numeric($_POST['id']) && $_POST['id'] > 0)
                $_SESSION['msg_success'] = 'Los detalles del producto se han actualizado con éxito';
            else
                $_SESSION['msg_success'] = 'Se ha agregado un nuevo producto con éxito';
            header('location: ./');
            exit;
        }
    } else {
...
<head>
    ...
    <title>Agregar productos a la tienda</title>
...
<body>
    <nav class="navbar navbar-expand-lg navbar-dark bg-danger bg-gradient">
        <div class="container">
            <a class="navbar-brand" href="./">Agregar nuevos productos</a>
        </div>
    </nav>
                    ...
                    <div class="card rounded-0 shadow">
                        <div class="card-header">
                            <div class="d-flex justify-content-between">
                                <div class="card-title col-auto flex-shrink-1 flex-grow-1">Descripción general</div>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="container-fluid">
                                <?php if (isset($data->id)) : ?>
                                    <p class="text-muted"><i>Editar <b><?= isset($data->name) ? $data->name : '' ?></b></i></p>
                                <?php else : ?>
                                    <p class="text-muted"><i>Nuevo producto</i></p>
                                <?php endif; ?>
                                <form id="member-form" action="" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-3">
                                        <label for="contact" class="form-label">Precio</label>
                        ...
                        <div class="card-footer text-center">
                            <button class="btn btn-danger rounded-0" form="member-form"><i class="fa-solid fa-save"></i> Guardar nuevo producto</button>
